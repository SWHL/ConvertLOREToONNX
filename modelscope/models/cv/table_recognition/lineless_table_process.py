...
def _get_4ps_feat(cc_match, output):
    ...
    if not (isinstance(output, dict)):
        cc_match = torch.where(
            ...
            * torch.ones(cc_match.shape).to(torch.int64))
        cc_match = torch.where(
            cc_match >= 0, cc_match,
            torch.zeros(cc_match.shape).to(torch.int64))
...
def _topk(scores, K=40):
    ...
    topk_inds = topk_inds % (
        torch.Tensor([height]).to(torch.int64)
        * torch.Tensor([width]).to(torch.int64))
    topk_ys = (topk_inds / torch.Tensor([width])).int().float()
    topk_xs = (topk_inds
               % torch.Tensor([width]).to(torch.int64)).int().float()
...
def normalized_ps(ps, vocab_size):
    ...
    ps = torch.where(ps < vocab_size, ps, (vocab_size - 1)
                     * torch.ones(ps.shape).to(torch.int64))
